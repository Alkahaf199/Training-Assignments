...
object WordCountBetterSortedDataset {
  ...
  def main (args: Array[String]): Unit = {
    ...
    import spark.implicits._
    val input = spark.read.text(path = "data/book.txt").as[Book]

    val words = input.select(explode(split($"value", "\\W+")).alias("Words")).filter($"Words" =!= "")

    val lowerCase = words.select(lower($"Words").alias("Words"))
    val wordCount = lowerCase.groupBy("Words").count()
    val sortedWordCount = wordCount.sort("count")
    sortedWordCount.show(sortedWordCount.count.toInt)
